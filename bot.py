...
def validate_image_dimensions(raw: bytes, filename: str = "file") -> str | None:
    """
    Cek apakah dimensi gambar tidak melebihi 1080×1080.
    Return pesan error jika gagal, None jika OK.
    """
    try:
        ...
        if w > MAX_IMAGE_SIZE or h > MAX_IMAGE_SIZE:
            return (
                f"❌ Dimensi *{filename}* terlalu besar ({w}×{h}px).\n"
                f"Maksimal dimensi: *{MAX_IMAGE_SIZE}×{MAX_IMAGE_SIZE}px*."
            )
    except Exception:
        pass  # biarkan proses berikutnya yang handle
    return None


def validate_video_dimensions(raw: bytes, filename: str = "file") -> str | None:
    """
    Cek dimensi video via ffprobe, max 1080×1080.
    Return pesan error jika gagal, None jika OK.
    """
    try:
        ...
        if streams:
            ...
            if w > MAX_IMAGE_SIZE or h > MAX_IMAGE_SIZE:
                return (
                    f"❌ Dimensi video *{filename}* terlalu besar ({w}×{h}px).\n"
                    f"Maksimal dimensi: *{MAX_IMAGE_SIZE}×{MAX_IMAGE_SIZE}px*."
                )
    except Exception:
        pass  # skip jika ffprobe tidak tersedia
    return None
